...
public class GeekDevValidator {
    ...
    public static void saveTopic(Topic topic) {
        ...
        Validators.notEmpty(ErrorCode.PARAMETER_IS_MISS.getMsg()).and(between(5, 50)).test(topic.getTitle()).throwIfInvalid("标题");
        Validators.notEmpty(ErrorCode.PARAMETER_IS_MISS.getMsg()).and(between(5, 50_000)).test(topic.getContent()).throwIfInvalid("内容");
